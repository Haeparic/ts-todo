...
const AppContainer = () => {
  ...
  const getLocalData = async () => {
    ...
    if (data !== null) {
      ...
      const initData = firebaseData.map((item) => {
        // firebase 에서 가져온 데이터를 TypeScript 에서 우리가 만든 Type 으로 형변환하기
  ...
  const addTodo = async (
    ...
    // 새로운 todoType 생성
    // 기존 todoList state 를 복사하고, 추가 todoList 를 합쳐주고
    // todoList state 를 업데이트한다.
    // 이때, immutable 을 유지한다.
    // 필수 라이브러리로 Immer 가 있다.
    // Immer 는 객체의 불변성을 유지하는 것으로 업무에서 필수로 활용한다.
  ...
  const deleteTodo = async (todo: TodoType) => {
    ...
    let newTodoList = produce(todoList, (draft) => {
      // index 의 순서로부터 1개를 제거하고 나머지 배열을 리턴한다.
